...
SELECT EMP_ID, EMP_NAME, JOB_NAME, JOB_CODE, SALARY, HIRE_DATE 
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
FROM EMPLOYEE
GROUP BY DEPT_CODE; -- HAVING SALARY = MAX(SALARY); 
...
-- 선택함수 사용!
--> DECODE(컬럼명 , 값 1, 1인경우 , 값2, 2인경우,...일치하지 않는경우)
-- CASE WHEN 조건1 THEN 값1
--      WHEN 조건2 THEN 값2
-- 		ELSE 값
-- END 별칭  

SELECT EMP_ID,  EMP_NAME, DEPT_TITLE, JOB_NAME , MANAGER_ID,
...
FROM EMPLOYEE 
WHERE (JOB_CODE ,DEPT_CODE) = (SELECT JOB_CODE ,DEPT_CODE FROM EMPLOYEE WHERE EXTRACT(YEAR FROM HIRE_DATE) ='2000');
...
SELECT EMP_NAME, JOB_CODE , SALARY 
FROM EMPLOYEE MAIN
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE SUB WHERE SUB.JOB_CODE = MAIN.JOB_CODE);

-- 부서별 입사일이 가장 빠른 사원의
--    사번, 이름, 부서명(NULL이면 '소속없음'), 직급명, 입사일을 조회하고
--    입사일이 빠른 순으로 조회하세요
--    단, 퇴사한 직원은 제외하고 조회하세요
SELECT EMP_ID, EMP_NAME , NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE , ENT_YN 
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
...
SELECT EMP_ID, EMP_NAME , NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE , ENT_YN ,JOB_CODE
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
JOIN JOB USING(JOB_CODE);
...
-- 모든 직원의 이름, 직급, 급여, 전체 사원 중 가장 높은 급여 차 
SELECT EMP_NAME, JOB_CODE, SALARY,
...
SELECT EMP_ID, EMP_NAME,  NVL(MANAGER_ID, '없음') , NVL((SELECT EMP_NAME FROM EMPLOYEE SUB WHERE MAIN.MANAGER_ID = SUB.EMP_ID) , '없음')
FROM EMPLOYEE MAIN;
...
SELECT DENSE_RANK() OVER(ORDER BY SALARY DESC) 급여순위, EMP_NAME , SALARY 
FROM EMPLOYEE;
...
SELECT RANK() OVER(ORDER BY SALARY DESC) 급여순위, EMP_NAME , SALARY 
FROM EMPLOYEE;

SELECT 급여순위, EMP_NAME , SALARY 
FROM (SELECT RANK() OVER(ORDER BY SALARY DESC) 급여순위, EMP_NAME , SALARY 
FROM EMPLOYEE) WHERE 급여순위 <= 5;
