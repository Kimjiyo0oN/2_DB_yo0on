...
WHERE DEPARTMENT_NO =(SELECT DEPARTMENT_NO FROM TB_DEPARTMENT WHERE STUDENT_NAME = '치의학과')
ORDER BY STUDENT_NAME;
...
JOIN TB_PROFESSOR USING(PROFESSOR_NO)
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
WHERE CATEGORY = '인문사회';  ---에러나는 쿼리문
-- ORA-00918: column ambiguously defined 컬럼중복 에러
--TB_DEPARTMENT,TB_PROFESSOR, TB_CLASS  -> 모두 DEPARTMENT_NO를 가지고 있어서 
...
SELECT STUDENT_NO ,STUDENT_NAME
FROM (SELECT STUDENT_NO ,STUDENT_NAME, AVG(POINT) 순위
			FROM TB_STUDENT
			JOIN TB_GRADE USING(STUDENT_NO)
			JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
			WHERE DEPARTMENT_NAME ='국어국문학과'
...
JOIN TB_CLASS USING(CLASS_NO)
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
WHERE CATEGORY =(SELECT CATEGORY FROM TB_DEPARTMENT WHERE DEPARTMENT_NAME ='환경조경학과') AND CLASS_TYPE LIKE '전공%'
