 ...
 * 
 * 컬럼명 AS "별칭" : 별칭 띄어쓰기X, 특수문자O, 문자만 O
 * 
 * AS는 생략 가능 
 * 
 * 
...
-- 쌍따옴표 안에 작성되는 것들이 하나의 (단어이다!)임를 의미 
...
--2개 다 중복 값을 제거해주는 걸로하면 중복값이 두개 모두 합쳐있을 때 없는 걸로 join됨??
--D9 J1
--D9 J2 로 출력
...
FROM EMPLOYEE
WHERE DEPT_CODE  = 'D9'; -- 같다 비교 연산자 (=) / 대입 연산자(:=)
...
-- 연습문제
-- EMPLOYEE 테이블에서 이메일 '_' 앞이 4 글자 이면서
-- 부서코드가 'D9' 또는 'D6' 이고 
 ...
 * 1. FROM절
 * 2. WHERE절
 * 4. ORDER BY절 컬럼명 | 별칭 | 컬럼 순서 [ASC | DESC] [NULLS FIRST|LAST]
 * 										   [오름 | 내림]
 */
...
FROM EMPLOYEE 
ORDER BY SALARY;  -- ASC 가 기본값

-- EMPLOYEE 테이블 급여 200만 이상인 사원의
-- 사번, 이름, 급여 조회
-- 단, 급여 내림 차순으로 조회
SELECT EMP_ID ,EMP_NAME ,SALARY 
FROM EMPLOYEE
WHERE SALARY >= 2000000 ORDER BY 3 DESC; --SALARY DESC ;

-- 입사일 순서대로 이름, 입사일 조회(별칭 사용)
SELECT EMP_NAME 이름,HIRE_DATE AS 입사일
FROM EMPLOYEE
ORDER BY 입사일;

/*정렬 종첩 : 대분류 정렬 후 소분류 정렬*/
--부서 코드 오름차순 정렬 후 급여 내림차순 정렬
SELECT EMP_NAME , DEPT_CODE ,SALARY 
FROM EMPLOYEE
ORDER BY DEPT_CODE DESC --, SALARY DESC; --ORDER BY DEPT_CODE 
