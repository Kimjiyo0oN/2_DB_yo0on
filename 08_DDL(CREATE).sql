...
-- SQL 작성법 : 대문자 작성을 권장, 연결된 단어 사이에는 "_" (언더바) 사용
...
INSERT INTO  MEMBER1 VALUES (12345.07);
INSERT INTO  MEMBER1 VALUES (12345.001);
INSERT INTO  MEMBER1 VALUES (12345.061);
...
--*INSERT/UPDATE 시 컬럼 값으로 DEFAULT를 작성하...........................................................................
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER" VALUES('MEM02','QWER1234','김영희','950110-2234567',SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO "MEMBER" VALUES('MEM03','MEM03DLEK',' 김찬희','911110-1234567',DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO "MEMBER"(MEMBER_ID,MEMBER_PWD,MEMBER_NAME,MEMBER_SSN) VALUES('MEM04','04smswlqdmfh',' 김호수','910710-1234567');

-- ** JDBC에서 날짜를 입력 받았을 때 삽입하는 방법 **
INSERT INTO "MEMBER" VALUES('MEM05','PASS05', '김길동', '930303-1333333',TO_DATE('2022-09-13 17:33:27','YYYY-MM_DD HH24:MI:SS'));
																										-- ?(PLACE HOLDER)
...
INSERT INTO MEMBER2 VALUES('MEM02','PASS02','이길동',0102341234);
--> NUMBER 타입 컬럼에 데이터 삽입시 
-- 제일 앞에 0이 있으면 이를 자동으로 제거함 
   --> 전화번호, 주민등록번호 처럼 숫자로만 되어있는 데이터지만
   -- 0으로 시작할 가능성이 있으면 CHAR,VARCHAR2......................................................
...
-- * 테이블 레벨 : 테이블 생성 시 컬러 정의가 끝난 후 마지막에 작성
...
--    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_UNI UNIQUE,
    							-- 컬럼 레벨(제약조건명 지정)
    USER_ID VARCHAR(20),
    USER_PWD VARCHAR2(30),
    USER_NAME VARCHAR2(30),
    ...
    CONSTRAINT USER_ID_UNI UNIQUE(USER_ID)  
);
...
AND UCC.CONSTRAINT_NAME = 'USER_ID_UNI';
...
CREATE TABLE USER_USED_PK(
    ...
    EMAIL VARCHAR2(50)
	--테이블 레벨
    --CONSRAINT USER_NO_PK PRIMARY KEY(USER_NO)
);
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KJY.PK_USERNO_USERID)에 위배됩니다
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_PK REFERENCES USER_GRADE/*(GRADE_CODE)*/ --컬럼 레벨 
  										-- 컬럼명 미작성 시 USER_GRADE 테이블의 PK를 자동 참조 
  -- 테이블 레벨 
  --  , CONSTRAINT GRADE_CODE_PK FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE
...
--SQL Error [2292] [23000]: ORA-02292: 무결성 제약조건(KH_KJY.GRADE_CODE_PK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
CREATE TABLE USER_USED_FK3_CP(
  ...
  CONSTRAINT GRADE_CODE_FK3_CP FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE3 ON DELETE CASCADE
);
...
--SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(KH_KJY.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
	...
	GENDER CHAR(2) CONSTRAINT CK_GENDER CHECK(GENDER IN('남','여')),
	PHONE VARCHAR2(11),
	ADDRESS VARCHAR2(30),
	STATUS CHAR(2) DEFAULT 'N' CONSTRAINT NN_STATUS NOT NULL,
...
-- 테이블안의 여러컬럼에 선언 가능
-- 예를들어, 회원가입테이블에서 이름과 전화번호가 필수인 경우 
-- Not Null 제약조건을 걸어서 필수사항을 꼭 기입하도록 하는 조건이다
...
ALTER TABLE LOCATION ADD CONSTRAINT NAT_NATIONAL_CODE
