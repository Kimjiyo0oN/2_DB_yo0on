...
FROM EMPLOYEE --GROUP BY DEPT_CODE;  --> DEPT_CODE가 같은 행 끼리 하나의 그룹이 됨
--SQL Error [937] [42000]: ORA-00937: 단일 그룹의 그룹 함수가 아닙니다

-- EMPLOYEE 테이블에서
-- 직급코드가 같은 사람의 직급 코드, 급여 평균, 인원 수를
-- 직급코드 오름 차순으로 조회
...
SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') 성별 ,SUM(DECODE(SUBSTR(EMP_NO,8,1),'1',1,1))
FROM EMPLOYEE
GROUP BY SUBSTR(EMP_NO,8,1);
...
GROUP BY JOB_CODE;--TO_CHAR(HIRE_DATE,'YYYY-MM-DD')
...
-- EMPLOYEE 테이블에서 직급별 인원수가 5명 이하인 직급코드, 인원 수 조회(직급코드 오르차순)
SELECT JOB_CODE , COUNT(*) 
FROM EMPLOYEE 
GROUP BY JOB_CODE 
HAVING COUNT(JOB_CODE) <= 5
ORDER BY 1;
...
-- ROLL UP : GROUP BY 절에서 가장 먼저 작성된 컬럼의 중간 집계를 처리하는 함수

SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY ROLLUP(DEPT_CODE, JOB_CODE)  --DEPT_CODE에 D1,  D2....에 각각 합한 값과 전체 DEPT_CODE, JOB_CODE 합한 계를 출력
ORDER BY 1;							  -- D1 NULL 합계                                  NULL       NULL 합계 							

SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY CUBE(DEPT_CODE, JOB_CODE); --DEPT_CODE,JOB_CODE 각각에 합한 값과 전체 DEPT_CODE, JOB_CODE  합한 계를 출력
--ORDER BY 1;					     -- D1 NULL 합계   NULL J6 합계             NULL       NULL 합계 	
