...
-- JOIN ( 합석ㅋㅋ )
-- 하나 이상의 테이블에서 데이터를 조회하기 위해 사용. FROM절에 EMPLOYEE 외에 더 추가..ㅠ
-- 수행 결과는 하나의 Result Set으로 나옴.

--(참고) JOIN은 서로 다른 테이블의 행을 하나씩 이어 붙히기 때문에 
--         [시간이 오래 걸리는 단점이 있다.]
...
- 관계형 데이터베이스는 최소한의 데이터를 테이블에 담고 있어
  ...
  두 테이블에서 같은 데이터를 저장하는 컬럼이 연결고리가됨.   EMPLOYEE테이블에서 'D9'와 DEPARTENT테이블에서 'D9'
*/
...
--JOIN DEPARTMENT ON (연결고리 컬럼);


-- 1. 내부 조인(INNER JOIN) ( == 등가 조인(EQUAL JOIN))
--> 연결되는 컬럼의 값이 일치하는 행들만 조인됨.  (== 일치하는 값이 없는 행은 조인에서 제외됨. 'NULL' )
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE); -- ANSI에서 연결할 두개의 컬럼명이 같으면 ON대신 USING사용후 (컬럼명) 한번만 씀
...
WHERE E.JOB_CODE = J.JOB_CODE; -- 오라클에서 연결할 두개의 컬럼명이 같으면 컬럼명 앞에 테이블명 부여/ SELECT 컬럼에도 한가지 테이블명 부여
                                                   -- FROM 테이블명 뒤에 맨앞 한글자를 별칭으로 줌으로써 테이블명을 줄일수 있다.
...
-- 2. 외부 조인(OUTER JOIN) NULL같은 집계되지 않은 부분을 조회 할 때
...
SELECT EMP_NAME, DEPT_TITLE
FROM DEPARTMENT
RIGHT JOIN EMPLOYEE ON (DEPT_CODE = DEPT_ID ); --위에랑 같은 코드--23행(하동운, 이오리 포함)
...
-- 오라클 --outer-join된 테이블은 1개만 지정할 수 있습니다 --에러
/*SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE , DEPARTMENT
WHERE DEPT_CODE(+) = DEPT_ID(+);*/
...
-- ANSI 표준
SELECT E1.EMP_ID , E1.EMP_NAME ,NVL(E1.MANAGER_ID,'없음') AS 사수ID, NVL(E2.EMP_NAME,'-') AS 사수이름
FROM EMPLOYEE E1
...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE); --DEPARTMENT 테이블에 LOCATION_ID
...
AND LOCATION_ID = LOCAL_CODE; -- (EMPLOYEE + DEPARTMENT) 
...
WHERE EMPLOYEE.DEPT_CODE  = DEPARTMENT.DEPT_ID AND EMPLOYEE.JOB_CODE = JOB.JOB_CODE
AND DEPARTMENT.LOCATION_ID = LOCATION.LOCAL_CODE AND JOB_NAME = '대리' 
AND LOCAL_NAME LIKE 'ASIA%';
...
JOIN DEPARTMENT ON (DEPT_CODE =DEPT_ID)
JOIN JOB ON (EMPLOYEE.JOB_CODE=JOB.JOB_CODE)
WHERE (SUBSTR( EMP_NO, 1, 2) >=70 AND SUBSTR( EMP_NO, 1, 2) < 80) AND EMPLOYEE.JOB_CODE = JOB.JOB_CODE
AND EMP_NAME LIKE '전%';
...
AND LOCATION.LOCAL_CODE = DEPARTMENT.LOCATION_ID
ORDER BY 3;

SELECT EMP_NAME , JOB_NAME, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE 
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)  -- 이너 조인 이라서 NULL값 여기서 다 빠짐
...
SELECT EMP_NAME, JOB_NAME, SALARY, (SALARY*(NVL(BONUS,0) +1))*12 AS "연봉(보너스포함)"
FROM EMPLOYEE, JOB, SAL_GRADE
WHERE EMPLOYEE.JOB_CODE = JOB.JOB_CODE 
AND SAL_GRADE.SAL_LEVEL = EMPLOYEE.SAL_LEVEL
AND SALARY > SAL_GRADE.MIN_SAL;
...
NATURAL JOIN JOB
JOIN SAL_GRADE USING(SAL_LEVEL)
WHERE SALARY > SAL_GRADE.MIN_SAL;  --WHERE SALARY > MIN_SAL;
...
SELECT EMP_NAME , DEPT_TITLE, LOCAL_NAME, NATIONAL.NATIONAL_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
WHERE NATIONAL_CODE IN ('KO', 'JP') --WHERE NATIONAL_NAME IN ('한국', '일본')
ORDER BY 4;
