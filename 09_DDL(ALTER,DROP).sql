...
-- 객체(OBJECT)를 만들고(CREATE), 수정하고(ALTER), 삭제(DROP)하는 데이터 정의 언어
...
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_CP_DEPT_TI_UNIQUE UNIQUE(DEPT_TITLE);


-- DEPT_COPY의 DEPT_TITLE 컬럼에 UNIQUE 삭제
ALTER TABLE DEPT_COPY DROP CONSTRAINT DEPT_CP_DEPT_TI_UNIQUE;

--*** DEPT_COPY의 DEPT_TITLE 컬럼에 NOT NULL 제약조건 추가/삭제***
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_CP_DEPT_TI_NOTNULL NOT NULL(DEPT_TITLE);
--> NOT NULL 제약조건은 새로운 조건을 추가하는 것이 아닌
-- 컬럼 자체에 NULL 허용/비허용을 제어하는 성질 변경의 형태로 인식됨.

-- MODIFY(수정하다) 구문을 사용해서 NULL 제어
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE CONSTRAINT DEPT_CP_DEPT_TI_NOTNULL NOT NULL;

ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE CONSTRAINT DEPT_CP_DEPT_TI_NOTNULL NULL;
...
--> 컬럼이 생성되면서 DEFAULT 값이 자동 삽입 됨

--D10 개발 1팀 추가
INSERT INTO DEPT_COPY 
VALUES('D10','개발1팀','L1',DEFAULT,DEFAULT);
-- "KH_KJY"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)

--DEPT_ID 컬럼 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3);
--  DEPT_ID VARCHAR2(2)에서 3으로 수정 
--> 다시 위 INSERT 실행 -> 성공
...
-- DROP TABLE 테이블명 [CASCADE DONSTRAINTS];
...
INSERT INTO TB2 VALUES(111,1);
INSERT INTO TB2 VALUES(12,2);
INSERT INTO TB2 VALUES(13,3);
INSERT INTO TB2 VALUES(13,4);
--SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KJY.SYS_C008422)에 위배됩니다
...
-- 3) DROP TABLE 삭제 옵션 CASCASDE CONSTRAINTS 사용
	--> CASCADE CONSRAINTS
