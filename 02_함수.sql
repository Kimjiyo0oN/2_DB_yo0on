...
-- 함수는 SELECT 문의 
-- SELECT 절, WHERE 절, ORDER BY절, GROUP BY절, HAVING 절 사용가능 
...
SELECT EMP_ID,EMP_NAME 
FROM EMPLOYEE
WHERE MOD(EMP_ID,2) != 0 -- <> 0;

-- ROUND(숫자 | 컬럼명 [,소수점 위치]) : 반올림
SELECT ROUND(123.456) FROM DUAL; --123 , 수소점 첫 번째 자리에서 반 올림

SELECT ROUND(123.456 ,1) FROM DUAL; --123.5 , 수소점 첫 번째 자리에서 반 올림
									--> 두 번째 자리에서 반올림해서
									-- 소수점 한 자리만 표현 
	
SELECT ROUND(123.456 ,0) FROM DUAL; -- 소수점 0번째 자리에서 반올림(0 기본값)

SELECT ROUND(123.456 ,-2) FROM DUAL; -- 소수점 0번째 자리에서 반올림해서 
									 --소수점 -2자리 표현
...
SELECT TRUNC(123.456, 2) FROM DUAl; --123.4, 10의 자리 아래 버림 
...
SELECT SYSDATE  + 120 FROM DUAL; --2022-12-30 11:24:55.000
...
SELECT TO_CHAR(1000000,'L9,999,999') FROM DUAL; --'1,000,000원'
...
SELECT TO_CHAR(SYSDATE,'YYYY/MM/DD HH24:MI:SS DAY') FROM DUAL; --2022/09/01 11:25:28 목요일

SELECT TO_CHAR(SYSDATE,'MM/DD (DY)') FROM DUAL;-- 09/01 (목)
SELECT TO_CHAR(SYSDATE,'MM/DD (DAY)') FROM DUAL;-- 09/01 (목요일)
...
SELECT SYSDATE FROM DUAL; --2022-09-01 11:31:46.000

SELECT '2022-09-02' FROM DUAL; --2022-09-02
SELECT TO_DATE('2022-09-02') FROM DUAL; --2022-09-02 00:00:00.000

SELECT 20220902 FROM DUAL; --20220902
SELECT TO_DATE(20220902) FROM DUAL; --2022-09-02 00:00:00.000

SELECT TO_DATE('220901 113255') FROM DUAL; --ORA-01861: 리터럴이 형식 문자열과 일치하지 않음
SELECT TO_DATE('220901 113255', 'YYMMDD HH24MISS') FROM DUAL;
--> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜 형식인지 인지시킴

-- EMPLOYEE 테이블에서 각 직원이 태어난 생년월일(1990년 05년 13일) 조회
...
-- EMPLOYEE 테이블에서 
-- 급여가 500만원 이상이면 '고'
-- 급여가 300만원 이상 500미만이면 '중'
-- 급여가 300만원 미만이면 '소'으로 조회

SELECT EMP_NAME , SALARY ,
	CASE 
		WHEN SALARY >= 5000000 THEN '대'  --if
		WHEN SALARY >= 3000000 THEN '중'  --IF else
		ELSE '소'
	END "급여 받는 정도"
FROM EMPLOYEE;
--------------------------------------------------------------------------------
...
-- AVG(숫자가 기록된 컬럼명)  : 평균
-- 전 직원 급여 평균
SELECT ROUND(AVG(SALARY)) FROM EMPLOYEE; --ROUND : 소수점 반올림
...
FROM EMPLOYEE 
WHERE SALARY = (SELECT MAX(SALARY) FROM EMPLOYEE);-- 서브쿼리 + 그룹함수
...
SELECT COUNT(DISTINCT DEPT_CODE) FROM EMPLOYEE; --NULL 값 포함 X
...
SELECT SUM(DECODE(SUBSTR(EMP_NO,8,1), '2' , 1,0))
FROM EMPLOYEE;

SELECT DECODE(SUBSTR(EMP_NO,8,1), '2' , 1,0)
FROM EMPLOYEE;
...
SELECT SUM(DECODE(SUBSTR(EMP_NO,8,1), '2' , 1,0))
FROM EMPLOYEE 
GROUP BY DECODE(SUBSTR(EMP_NO,8,1), '2' , 1,0);
